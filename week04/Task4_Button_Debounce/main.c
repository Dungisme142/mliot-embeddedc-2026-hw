...
int main(void)
{
  ...
  while (1)
  {
    /* ------------------- TASK 4: QUÉT NÚT NHẤN CHỐNG DỘI TẠI CHÂN PA0 ------------- */
    /* Đọc trạng thái tức thời của chân PA0 (Nút nhấn) */
    uint8_t raw_reading = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);

    /* 1. Giải thuật Chống dội (Debounce) */
    if (raw_reading != last_button_state)
    {
    ...
    {
      if (raw_reading != button_stable_state)
      {
        button_stable_state = raw_reading;

        /* 2. Giải thuật Chống giữ (Edge Detection): Kích hoạt khi vừa nhấn nút (Sườn xuống) */
        if (button_stable_state == 0)
        {
          /* Đảo trạng thái LED PC13 ngay lập tức khi nhấn phím */
          HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
          
          /* Đồng bộ mốc thời gian chớp tắt tự động của Task 3 */
          last_blink_time = HAL_GetTick();
        }
      }
    }

    last_button_state = raw_reading;
